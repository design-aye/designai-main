...
// ========================================
// FILE PERSISTENCE AND VERSION HISTORY
// ========================================

/**
 * AppFiles table - Persisted current file state for each app.
 * Updated whenever the agent saves or regenerates a file.
 */
export const appFiles = sqliteTable('app_files', {
    ...
    appPathIdx: uniqueIndex('app_files_app_path_idx').on(table.appId, table.filePath),
    appIdx: index('app_files_app_idx').on(table.appId),
}));

/**
 * AppSnapshots table - Named restore points (version history) for an app.
 * Created automatically on generation complete, deploy, or manually by user.
 */
export const appSnapshots = sqliteTable('app_snapshots', {
    ...
    trigger: text('trigger', {
        enum: ['generation', 'deploy', 'manual', 'fork_source'],
    }).notNull().default('manual'),
    fileCount: integer('file_count').notNull().default(0),

    createdAt: integer('created_at', { mode: 'timestamp' }).default(sql`CURRENT_TIMESTAMP`),
}, (table) => ({
    appIdx: index('app_snapshots_app_idx').on(table.appId),
    appCreatedAtIdx: index('app_snapshots_app_created_at_idx').on(table.appId, table.createdAt),
}));

/**
 * AppSnapshotFiles table - File contents captured in a specific snapshot.
 */
export const appSnapshotFiles = sqliteTable('app_snapshot_files', {
    id: text('id').primaryKey(),
    snapshotId: text('snapshot_id').notNull().references(() => appSnapshots.id, { onDelete: 'cascade' }),
    appId: text('app_id').notNull().references(() => apps.id, { onDelete: 'cascade' }),
    ...
    snapshotPathIdx: uniqueIndex('app_snapshot_files_snapshot_path_idx').on(table.snapshotId, table.filePath),
    snapshotIdx: index('app_snapshot_files_snapshot_idx').on(table.snapshotId),
    appIdx: index('app_snapshot_files_app_idx').on(table.appId),
}));
...
export type NewAppFile = typeof appFiles.$inferInsert;

export type AppSnapshot = typeof appSnapshots.$inferSelect;
export type NewAppSnapshot = typeof appSnapshots.$inferInsert;

export type AppSnapshotFile = typeof appSnapshotFiles.$inferSelect;
export type NewAppSnapshotFile = typeof appSnapshotFiles.$inferInsert;
