        ...
        for (const file of files) {
            ...
            if (oldFile) {
                try {
                    ...
                    if (lastDiff) {
                        logger.debug(`Generated diff for file ${file.filePath}:`, { diff: lastDiff });
                    }
                } catch (error) {
                    logger.error(`Failed to generate diff for file ${file.filePath}:`, error instanceof Error ? error : new Error(String(error)));
                }
            }
