...
const logger = createLogger('InferUtils');
import {
    type SchemaFormat,
...
function accumulateToolCallDelta(
    ...
    if (idFromDelta && byId.has(idFromDelta)) {
        ...
        logger.debug(`[TOOL_CALL_DEBUG] Found existing entry by id: ${idFromDelta}`);
    } else if (idx !== undefined && byIndex.has(idx)) {
        entry = byIndex.get(idx)!;
        logger.debug(`[TOOL_CALL_DEBUG] Found existing entry by index: ${idx}`);
    } else {
        logger.debug(`[TOOL_CALL_DEBUG] Creating new entry - id: ${idFromDelta}, index: ${idx}`);
        // Create new entry
    ...
    if (deltaToolCall.function?.arguments !== undefined) {
        ...
        if (!isComplete) {
            ...
            logger.debug(`[TOOL_CALL_DEBUG] Accumulating arguments for ${entry.function.name || 'unknown'}:`, {
                id: entry.id,
                index: entry.index,
...
export async function buildGatewayUrl(env: Env, providerOverride?: AIGatewayProviders): Promise<string> {
    ...
    if (env.CLOUDFLARE_AI_GATEWAY_URL &&
        ...
        try {
            ...
            logger.warn(`Invalid CLOUDFLARE_AI_GATEWAY_URL provided: ${env.CLOUDFLARE_AI_GATEWAY_URL}. Falling back to AI bindings.`);
        }
    }
    ...
    return Promise.all(
        openAiToolCalls.map(async (tc) => {
            try {
                ...
                logger.debug(`Tool execution result for ${tc.function.name}:`, { result });
                return {
                    id: tc.id,
                ...
                logger.error(`Tool execution failed for ${tc.function.name}:`, error instanceof Error ? error : new Error(String(error)));
                return {
                    id: tc.id,
                    name: tc.function.name,
                    arguments: {},
...
export async function infer<OutputSchema extends z.AnyZodObject>({
    ...
    if (currentDepth >= MAX_TOOL_CALLING_DEPTH) {
        logger.warn(`Tool calling depth limit reached (${currentDepth}/${MAX_TOOL_CALLING_DEPTH}). Stopping recursion.`);
    ...
    try {
        ...
        logger.debug(`baseUrl: ${baseURL}, modelName: ${modelName}`);
        ...
        logger.debug(`Token optimization: Original messages size ~${JSON.stringify(messages).length} chars, optimized size ~${JSON.stringify(optimizedMessages).length} chars`);
        ...
        logger.info(`Running inference with ${modelName} using structured output with ${format} format, reasoning effort: ${reasoning_effort}, max tokens: ${maxTokens}, temperature: ${temperature}, baseURL: ${baseURL}`);
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                logger.info(`Inference response received on attempt ${attempt}`);
                break; // Success!

            } catch (error) {
                lastError = error;
                logger.error(`Attempt ${attempt} failed:`, error instanceof Error ? error : new Error(String(error)));
                ...
                if (!isDirectFallback) {
                    if ((modelName.includes('gemini') || modelName.includes('google')) && env.GOOGLE_AI_STUDIO_API_KEY) {
                        logger.warn('Attempting direct Gemini API fallback...');
                        ...
                        logger.warn('Attempting direct OpenAI API fallback...');
            ...
            if (response instanceof Stream) {
                ...
                for await (const event of response) {
                    ...
                    if (delta?.tool_calls && (provider === 'google-ai-studio' || provider === 'gemini')) {
                        logger.debug(`[PROVIDER_DEBUG] ${provider} tool_calls delta:`, { tool_calls: delta.tool_calls });
                    }
                    ...
                    if (delta?.tool_calls) {
                        try {
                            ...
                            logger.error('Error processing tool calls in streaming:', error instanceof Error ? error : new Error(String(error)));
                        }
                    }
                ...
                for (const toolCall of toolCalls) {
                    if (!toolCall.function.name) {
                        logger.warn('Tool call missing function name:', { toolCall });
                    }
                    if (toolCall.function.arguments) {
                        try {
                            // Validate JSON arguments early for visibility
                            const parsed = JSON.parse(toolCall.function.arguments);
                            logger.debug(`[TOOL_CALL_VALIDATION] Successfully parsed arguments for ${toolCall.function.name}:`, { parsed });
                        } catch (error) {
        ...
        if (!content && !stream && !toolCalls.length) {
            ...
            logger.warn('No content received from OpenAI', { response: JSON.stringify(response) });
        ...
        if (executedToolCalls.length) {
            logger.debug(`Tool calls executed:`, { executedToolCalls });
            ...
            logger.debug(`Tool calling depth: ${newDepth}/${MAX_TOOL_CALLING_DEPTH}`);
            ...
            if (executedCallsWithResults.length) {
                ...
                logger.debug('No tool calls with results');
        ...
        try {
            ...
            if (!result.success) {
                logger.warn('Schema validation failed', { rawContent: content, parsedData: parsedContent, errors: result.error.format() });
            ...
            logger.error('Error parsing response:', parseError instanceof Error ? parseError : new Error(String(parseError)));
        ...
        logger.error('Error in inferWithSchemaOutput:', error instanceof Error ? error : new Error(String(error)));
        throw error;
    }
}
