...
export class SimpleCodeGeneratorAgent extends Agent<Env, CodeGenState> {
    ...
    async generateAllFiles(reviewCycles: number = 5): Promise<void> {
        ...
        // Persist files to D1 and create an auto-snapshot for version history
        await this.persistFilesAndSnapshot('Initial generation', 'generation');
     ...
     */
    /**
     * Persist all current in-memory files to D1 and create a named snapshot.
     * Fire-and-forget: errors are logged but never bubble up to the caller.
     */
    private async persistFilesAndSnapshot(label: string, trigger: 'generation' | 'deploy' | 'manual'): Promise<void> {
        try {
            const filesMap = this.state.generatedFilesMap;
            if (!filesMap || Object.keys(filesMap).length === 0) return;

            const appId = this.getAgentId();
            const fileEntries = Object.values(filesMap).map(f => ({
                filePath: f.filePath,
                content: f.fileContents,
            }));

            const fileStorage = new FileStorageService(this.env);
            await fileStorage.upsertFiles(appId, fileEntries);

            const snapshotService = new SnapshotService(this.env);
            await snapshotService.createSnapshot(appId, fileEntries, label, trigger);

            this.logger().info(`Persisted ${fileEntries.length} files and created snapshot: "${label}"`);
        } catch (error) {
            this.logger().error('Failed to persist files / create snapshot:', error instanceof Error ? error : new Error(String(error)));
        }
    }
    ...
    async deployToCloudflare(): Promise<{ deploymentUrl?: string; workersUrl?: string } | null> {
        try {
            ...
            // Persist files and create a deploy snapshot for version history
