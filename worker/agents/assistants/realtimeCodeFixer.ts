...
import { createObjectLogger, createLogger } from "../../logger";

const moduleLogger = createLogger('RealtimeCodeFixer');
...
export function IsRealtimeCodeFixerEnabled(inferenceContext: InferenceContext): boolean {
    if (AGENT_CONFIG.realtimeCodeFixer.name !== AIModels.DISABLED) {
        moduleLogger.info("Realtime code fixer enabled");
        return true;
    }
    ...
    if (inferenceContext.userModelConfigs?.['realtimeCodeFixer'] && inferenceContext.userModelConfigs['realtimeCodeFixer'].name !== AIModels.DISABLED) {
        moduleLogger.info("Realtime code fixer enabled by user");
        return true;
    }

    moduleLogger.debug("Realtime code fixer disabled", { userModelConfigs: inferenceContext.userModelConfigs });
    return false;
}
