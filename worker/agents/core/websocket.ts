...
export function handleWebSocketMessage(agent: SimpleCodeGeneratorAgent, connection: Connection, message: string): void {
    try {
        ...
        switch (parsedMessage.type) {
            ...
            //         });
            //     break;
            case WebSocketMessageRequests.SYNC_PREVIEW: {
                const appId = agent.getAgentId();
                if (!appId) {
                ...
                fileStorage.getFiles(appId).then(dbFiles => {
                    if (dbFiles.length === 0) {
                        ...
                        return agent.deployToSandbox();
                    }
                    const updatedMap = { ...agent.state.generatedFilesMap };
                    for (const dbFile of dbFiles) {
                        updatedMap[dbFile.filePath] = {
                            ...(updatedMap[dbFile.filePath] ?? {
                                filePurpose: '',
                                lasthash: '',
                                lastmodified: 0,
                            ...
                            fileContents: dbFile.content,
                            lasthash: '',
                            lastmodified: Date.now(),
                        };
                    }
                    agent.setState({ ...agent.state, generatedFilesMap: updatedMap });
                    return agent.deployToSandbox();
                }).catch((error: unknown) => {
                    logger.error('Error during preview sync:', error);
                    sendError(connection, `Preview sync failed: ${error instanceof Error ? error.message : String(error)}`);
                });
                break;
            }
            default:
