...
export function parseMarkdownContent<OutputSchema extends z.AnyZodObject>(
    ...
    if (debug) {
        logger.debug("--- Cleaned Markdown Input ---", { cleanedMarkdown });
    ...
    try {
        ...
        if (debug) {
            logger.debug("--- MDAST Root ---", { root: JSON.stringify(root, (key, value) => key === 'position' ? undefined : value) });
    ...
    try {
        ...
        if (debug) {
            logger.debug("--- Section Tree ---", { sectionTree: JSON.stringify(sectionTree, (key, value) => key === 'position' ? undefined : value) });
    ...
    try {
        ...
        if (debug) {
            logger.debug("--- Draft Data (Before Validation) ---", { draftData });
    ...
    if (!validationResult.success) {
        logger.warn('--- Zod Validation Failed ---', { errors: validationResult.error.format() });
