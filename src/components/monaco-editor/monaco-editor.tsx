...
export const MonacoEditor = memo<MonacoEditorProps>(function MonacoEditor({
	...
	useEffect(() => {
		...
		const contentDisposable = editor.current.onDidChangeModelContent(() => {
			onChangeRef.current?.(editor.current!.getValue());
		});

		// Fire onSelectionChange when selection changes
		const selectionDisposable = editor.current.onDidChangeCursorSelection(() => {
	...
	useEffect(() => {
		if (editor.current && createOptions.value !== prevValue.current) {
			...
			// If the current editor content already matches the incoming value
			// the change originated from the editor itself via onChange — skip setValue
			// to preserve cursor position and selection.
			const currentEditorValue = editor.current.getValue();
			if (currentEditorValue !== createOptions.value) {
	...
	// Update readOnly dynamically without recreating the editor
	useEffect(() => {
		if (editor.current) {
			editor.current.updateOptions({ readOnly: createOptions.readOnly ?? true });
		}
