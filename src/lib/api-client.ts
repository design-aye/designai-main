...
class ApiClient {
	...
	// ===============================
	// File Editor API Methods
	// ===============================

	async listAppFiles(appId: string): Promise<ApiResponse<FilesListData>> {
		return this.request<FilesListData>(`/api/apps/${appId}/files`);
	}

	async upsertAppFile(appId: string, filePath: string, content: string): Promise<ApiResponse<FileUpsertData>> {
		return this.request<FileUpsertData>(`/api/apps/${appId}/files`, {
			method: 'PUT',
			body: { filePath, content },
		});
	}

	async deleteAppFile(appId: string, filePath: string): Promise<ApiResponse<FileDeleteData>> {
		return this.request<FileDeleteData>(`/api/apps/${appId}/files`, {
			method: 'DELETE',
			body: { filePath },
		});
	}
	...
	): Promise<ApiResponse<AiEditData>> {
		return this.request<AiEditData>(`/api/apps/${appId}/ai-edit`, {
			method: 'POST',
			body: { filePath, fileContent, instruction, selectionText },
		});
	}
	...
	async listSnapshots(appId: string): Promise<ApiResponse<SnapshotListData>> {
		return this.request<SnapshotListData>(`/api/apps/${appId}/versions`);
	}

	async createSnapshot(appId: string, label?: string): Promise<ApiResponse<SnapshotCreateData>> {
		return this.request<SnapshotCreateData>(`/api/apps/${appId}/versions`, {
			method: 'POST',
			body: label ? { label } : {},
		});
	}

	async restoreSnapshot(appId: string, snapshotId: string): Promise<ApiResponse<SnapshotRestoreData>> {
		return this.request<SnapshotRestoreData>(`/api/apps/${appId}/versions/${snapshotId}/restore`, {
			method: 'POST',
		});
	}
